#!/usr/bin/env python3
"""สร้างร่างบันทึกผลการจัดการเรียนรู้ภาษาไทยจากไฟล์หลักฐานประกอบแผน

เครื่องมือนี้ใช้เปรียบเทียบไฟล์ DOCX ตัวอย่าง/รายงานของครูกับไฟล์ DOCX
แผนการจัดการเรียนรู้เป้าหมาย แล้วสร้างร่าง "บันทึกผลการจัดการเรียนรู้"
เป็นภาษาไทย โดยตั้งใจใช้เฉพาะ Python standard library เพื่อให้รันได้ใน
สภาพแวดล้อมของโรงเรียนหรือการประเมินที่อาจติดตั้งแพ็กเกจเพิ่มเติมไม่ได้
"""
...
class ExtractedDocument:
    """ข้อความและตารางที่ดึงออกมาจากไฟล์ DOCX"""
...
class EvidenceSummary:
    """ข้อมูลสรุปคะแนนและหลักฐานที่ใช้ประกอบร่างบันทึก"""
...
def extract_docx(path: Path) -> ExtractedDocument:
    """ดึงย่อหน้าและตารางจากไฟล์ DOCX"""
...
def detect_sections(document: ExtractedDocument) -> dict[str, list[str]]:
    """จัดกลุ่มย่อหน้าที่อยู่ใกล้กันตามหัวข้อบันทึกหลังแผนที่พบบ่อย"""
...
def extract_lesson_metadata(document: ExtractedDocument) -> dict[str, str]:
    """ค้นหาข้อมูลสำคัญของแผนการจัดการเรียนรู้จากข้อความในไฟล์"""
    patterns = {
...
def _video_links_from_text(text: str) -> list[str]:
    video_pattern = (
        r"https?://\S*"
        r"(?:youtube|youtu\.be|drive\.google|facebook|tiktok|vimeo)"
        r"\S*"
    )
    return re.findall(video_pattern, text, re.I)
...
def summarize_evidence(paths: Iterable[Path]) -> EvidenceSummary:
    ...
    for path in paths:
        ...
        except Exception as exc:  # noqa: BLE001 - อ่านไฟล์อื่นต่อ แม้ไฟล์หลักฐานบางไฟล์มีปัญหา
...
def build_record(
    ...
    if not meta_lines:
        meta_lines = (
            "- **ข้อมูลแผน:** "
            "(ตรวจสอบชื่อหน่วย เรื่อง ชั้น และเวลาเรียนจากไฟล์แผนการจัดการเรียนรู้)"
        )
    ...
    for heading in ("ผลการจัดการเรียนรู้", "ปัญหาและอุปสรรค", "แนวทางแก้ไข"):
        if heading in sample_sections:
            style_reference.append(
                f"### ตัวอย่างรูปแบบ: {heading}\n"
                f"{_bulletize(sample_sections[heading], 3)}"
            )
    style_block = "\n\n".join(style_reference) or (
        "- ไม่พบหัวข้อบันทึกหลังแผนที่ชัดเจนในไฟล์ตัวอย่าง"
    )

    evidence_files = "\n".join(f"- {path}" for path in evidence.files) or (
        "- (ยังไม่ได้แนบไฟล์หลักฐานเพิ่มเติม)"
    )
    notes = "\n".join(f"- {note}" for note in evidence.notes) or (
        "- ไม่มีข้อผิดพลาดจากการอ่านไฟล์หลักฐาน"
    )
    learning_result = (
        "จากการจัดกิจกรรมการเรียนรู้ตามแผนการจัดการเรียนรู้ "
        "พบว่าผู้เรียนได้ฝึกคิด วิเคราะห์ และลงมือปฏิบัติกิจกรรมตามภาระงานที่กำหนด "
        "ผู้เรียนส่วนใหญ่สามารถอธิบายขั้นตอนการทำงาน แลกเปลี่ยนความคิดเห็น "
        "และสร้างชิ้นงาน/ใบงานได้สอดคล้องกับจุดประสงค์การเรียนรู้ "
        "โดยพิจารณาจากหลักฐานคะแนน ชิ้นงาน ใบงาน และคลิปวิดีโอการสอนที่แนบประกอบ"
    )
    problem_note = (
        "ผู้เรียนบางส่วนยังต้องได้รับการช่วยเหลือในการอ่านคำสั่ง วิเคราะห์โจทย์ "
        "หรือวางแผนลำดับขั้นตอนการทำงาน ครูจึงควรใช้คำถามชี้นำ "
        "ตัวอย่างชิ้นงาน และการสาธิตซ้ำเป็นรายกลุ่ม/รายบุคคล "
        "เพื่อให้ผู้เรียนสามารถทำงานได้ครบถ้วนมากขึ้น"
    )
    improvement_note = (
        "ในครั้งต่อไปควรจัดเตรียมใบงานที่มีลำดับขั้นตอนชัดเจน "
        "เพิ่มเกณฑ์การประเมินแบบตรวจสอบรายการ "
        "ใช้ตัวอย่างจากคลิปวิดีโอการสอนประกอบการทบทวน "
        "และจัดกิจกรรมช่วยเหลือผู้เรียนที่ยังไม่ผ่านเกณฑ์ด้วยการฝึกซ้ำหรือทำชิ้นงานปรับปรุง"
    )
    next_plan_note = (
        "ควรนำผลคะแนนและข้อสังเกตจากชิ้นงานมาแบ่งกลุ่มผู้เรียนตามระดับความพร้อม "
        "เพื่อออกแบบกิจกรรมเสริมสำหรับผู้เรียนที่ต้องการความช่วยเหลือ "
        "และกิจกรรมท้าทายสำหรับผู้เรียนที่ทำได้ตามเกณฑ์แล้ว"
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "วิเคราะห์ไฟล์ DOCX ตัวอย่างและแผนการจัดการเรียนรู้"
            "เพื่อร่างบันทึกผลการจัดการเรียนรู้"
        ),
    )
    parser.add_argument(
        "--sample",
        required=True,
        type=Path,
        help="ไฟล์ DOCX ตัวอย่าง/รายงานของครู",
    )
    parser.add_argument(
        "--lesson-plan",
        required=True,
        type=Path,
        help="ไฟล์ DOCX แผนการจัดการเรียนรู้ที่ต้องการเขียนบันทึก",
    )
    parser.add_argument(
        "--evidence",
        nargs="*",
        default=[],
        type=Path,
        help="ไฟล์คะแนน ใบงาน ชิ้นงาน ลิงก์วิดีโอ หรือหลักฐานเพิ่มเติม",
    )
    parser.add_argument(
        "--output",
        default=Path("บันทึกผลการจัดการเรียนรู้.md"),
        type=Path,
        help="ไฟล์ Markdown ผลลัพธ์",
    )
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if args.analysis_json:
            _ensure_parent_dir(args.analysis_json)
