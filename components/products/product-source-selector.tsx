...
import { searchProductCandidatesAction } from '@/actions/product-discovery';
import { INITIAL_PRODUCT_DISCOVERY_STATE } from '@/lib/products/discovery-state';
...
export function ProductSourceSelector({shopeeEnabled}:{shopeeEnabled:boolean}){
  ...
  return <section aria-labelledby="product-discovery-title" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
    <h2 className="text-xl font-bold" id="product-discovery-title">ค้นหาสินค้า</h2><p className="mt-1 text-sm text-slate-500">ค้นหาด้วย Mock AI หรือนำเข้าด้วยตนเอง โดยไม่ดึงข้อมูลสินค้าด้วยการ scraping</p>
