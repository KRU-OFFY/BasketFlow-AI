...
const fields = [
  ...
  ['image_url','Product image URL','url'],['price','ราคา','number'],['commission_rate','Commission rate','number'],
] as const;
...
export function ProductImportForm() {
  ...
  return <form action={action} className="space-y-4">
    ...
    {fields.map(([name,label,type]) => <label className="block" key={name}>
      ...
      <input className="input mt-1" defaultValue={state.fields?.[name]} name={name} type={type} required={name==='title'||name==='affiliate_link'} step={type==='number'?'0.01':undefined}/>
    </label>)}
