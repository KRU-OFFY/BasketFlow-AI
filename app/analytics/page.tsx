...
export default async function Analytics() {
  ...
  const projectCount=(field?:string,value?:string)=>{let q=supabase.from('review_projects').select('*',{count:'exact',head:true}).eq('user_id',user.id);if(field&&value)q=q.eq(field,value);return q;};
  ...
  const error=results.map(x=>x.error).find(Boolean);if(error)throw new Error(`โหลด Analytics ไม่สำเร็จ: ${error.message}`);
