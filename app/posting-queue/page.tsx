...
import { queueStatusLabel } from '@/lib/workflow/status';
...
export default async function Queue() {
  ...
  return <AppShell><h1 className="text-3xl font-bold">คิวเผยแพร่</h1>{!rows?.length?<Card className="mt-6 text-center"><p className="font-bold">ยังไม่มีรายการในคิวเผยแพร่</p><p className="mt-2 text-slate-500">โปรเจกต์ต้องผ่าน Final Compliance, การอนุมัติ, Affiliate disclosure และ AI Content Label</p></Card>:<div className="mt-6 space-y-4">{rows.map(row=><Card key={row.id}><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="font-bold">{row.review_projects?.title??'โปรเจกต์'}</p><p className="text-sm text-slate-500">สินค้า: {row.review_projects?.products?.title??'-'}</p></div><div className="flex items-center gap-3"><Badge tone={row.status==='failed'?'red':row.status==='published'?'green':'purple'}>{queueStatusLabel(row.status)}</Badge><Link className="text-sm font-bold text-orange" href={`/projects/${row.project_id}`}>ดูโปรเจกต์</Link></div></div></Card>)}</div>}</AppShell>;
}
