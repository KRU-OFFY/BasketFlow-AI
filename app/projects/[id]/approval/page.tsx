...
export default async function Approval({params}:{params:Promise<{id:string}>}) {
  ...
  const compliancePassed=check?.status==='PASS' && project.compliance_status==='PASS';
  const humanApproved=approval?.status==='approved' && project.approval_status==='approved';
