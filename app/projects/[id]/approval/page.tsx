...
export default async function Approval({params}:{params:Promise<{id:string}>}) {
  ...
  return <AppShell><ProjectWorkflowNav current="approval" projectId={id}/><h1 className="text-3xl font-bold">Human Approval Gate</h1><Card className="mt-6 max-w-3xl"><Badge tone={safe?'green':'orange'}>{safe?'พร้อมเข้าคิว':'Safety Gate Locked'}</Badge><ul className="mt-5 space-y-3">{rows.map(([label,ok])=><li className={ok?'text-emerald-700':'text-red-700'} key={label}>{ok?'ผ่าน':'ยังไม่ผ่าน'} — {label}</li>)}</ul>{!compliancePassed?<p className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">Media หรือสคริปต์มีการเปลี่ยนแปลงหลังผลตรวจล่าสุด กรุณา <Link className="font-semibold underline" href={`/projects/${id}/compliance?phase=final`}>รัน Final Safety Check ใหม่</Link> ก่อนอนุมัติ</p>:null}<div className="mt-6 grid gap-4 md:grid-cols-2"><WorkflowActionForm action={approve} confirmMessage="ยืนยันอนุมัติ Script, Final Compliance และ Media revision ปัจจุบันหรือไม่?" disabled={!compliancePassed} initialRequestId={crypto.randomUUID()} label="อนุมัติเวอร์ชันนี้" pendingLabel="กำลังอนุมัติ…"/><WorkflowActionForm action={reject} buttonClassName="btn btn-secondary mt-3" className="rounded-2xl border border-slate-200 p-3" initialRequestId={crypto.randomUUID()} label="ปฏิเสธ" pendingLabel="กำลังบันทึก…"><label className="label" htmlFor="reason">เหตุผลที่ปฏิเสธ</label><input className="input mt-1" id="reason" name="reason" placeholder="ระบุเหตุผลเพื่อแก้ไขรอบถัดไป"/></WorkflowActionForm></div>{safe?<div className="mt-5"><WorkflowActionForm action={queue} confirmMessage="ส่ง snapshot เวอร์ชันที่อนุมัติแล้วเข้า Publishing Queue หรือไม่?" initialRequestId={crypto.randomUUID()} label="ส่งเข้า Publishing Queue" pendingLabel="กำลังตรวจ Safety Gate…"/></div>:null}{approval?<p className="mt-4 text-sm text-slate-500">ผลอนุมัติล่าสุด: {approval.status}{approval.notes?` · ${approval.notes}`:''}</p>:null}</Card></AppShell>;
}
