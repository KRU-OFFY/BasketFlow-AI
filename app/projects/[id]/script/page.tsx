...
export default async function Script({params}:{params:Promise<{id:string}>}) {
  ...
  const [{data:brief,error:briefError},{data:script,error:scriptError}]=await Promise.all([supabase.from('ai_briefs').select('id').eq('project_id',id).eq('user_id',user.id).order('created_at',{ascending:false}).limit(1).maybeSingle(),supabase.from('scripts').select('*').eq('project_id',id).eq('user_id',user.id).order('created_at',{ascending:false}).limit(1).maybeSingle()]);
