...
export default async function Compliance({params}:{params:Promise<{id:string}>}) {
  ...
  const [{data:script},{data:check,error}]=await Promise.all([supabase.from('scripts').select('id').eq('project_id',id).eq('user_id',user.id).order('created_at',{ascending:false}).limit(1).maybeSingle(),supabase.from('compliance_checks').select('*').eq('project_id',id).eq('user_id',user.id).order('created_at',{ascending:false}).limit(1).maybeSingle()]);
  if(error) throw new Error(`โหลด Compliance ไม่สำเร็จ: ${error.message}`);
